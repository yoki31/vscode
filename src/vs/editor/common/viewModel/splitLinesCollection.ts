...
export interface IModelLineView {
	isVisible(): boolean;
	setVisible(isVisible: boolean): IModelLineView;
...
export class SplitLinesCollection implements IViewModelLinesCollection {
	...
	private modelLineViews!: IModelLineView[];

	/**
	 * Reflects the sum of the line counts of all model line views.
	*/
	private modelLineViewsLineCounts!: ConstantTimePrefixSumComputer;
	...
	private _constructLines(resetHiddenAreas: boolean, previousLineBreaks: ((LineBreakData | null)[]) | null): void {
		...
		for (let i = 0; i < lineCount; i++) {
			...
			this.modelLineViews[i] = line;
		}

		this._validModelVersionId = this.model.getVersionId();

		this.modelLineViewsLineCounts = new ConstantTimePrefixSumComputer(values);
	}
	...
	public setHiddenAreas(_ranges: Range[]): boolean {
		...
		let hiddenAreaStart = 1, hiddenAreaEnd = 0;
		let hiddenAreaIdx = -1;
		let nextLineNumberToUpdateHiddenArea = (hiddenAreaIdx + 1 < hiddenAreas.length) ? hiddenAreaEnd + 1 : this.modelLineViews.length + 2;

		let hasVisibleLine = false;
		for (let i = 0; i < this.modelLineViews.length; i++) {
			let lineNumber = i + 1;
			...
			if (lineNumber === nextLineNumberToUpdateHiddenArea) {
				...
				hiddenAreaStart = hiddenAreas[hiddenAreaIdx].startLineNumber;
				hiddenAreaEnd = hiddenAreas[hiddenAreaIdx].endLineNumber;
				nextLineNumberToUpdateHiddenArea = (hiddenAreaIdx + 1 < hiddenAreas.length) ? hiddenAreaEnd + 1 : this.modelLineViews.length + 2;
			}

			let lineChanged = false;
			if (lineNumber >= hiddenAreaStart && lineNumber <= hiddenAreaEnd) {
				// Line should be hidden
				if (this.modelLineViews[i].isVisible()) {
					this.modelLineViews[i] = this.modelLineViews[i].setVisible(false);
					lineChanged = true;
				}
			} else {
				hasVisibleLine = true;
				...
				if (!this.modelLineViews[i].isVisible()) {
					this.modelLineViews[i] = this.modelLineViews[i].setVisible(true);
					lineChanged = true;
				}
			}
			if (lineChanged) {
				let newOutputLineCount = this.modelLineViews[i].getViewLineCount();
				this.modelLineViewsLineCounts.setValue(i, newOutputLineCount);
			}
		}
	...
	public modelPositionIsVisible(modelLineNumber: number, _modelColumn: number): boolean {
		if (modelLineNumber < 1 || modelLineNumber > this.modelLineViews.length) {
			// invalid arguments
			return false;
		}
		return this.modelLineViews[modelLineNumber - 1].isVisible();
	}
	...
	public getModelLineViewLineCount(modelLineNumber: number): number {
		...
		return this.modelLineViews[modelLineNumber - 1].getViewLineCount();
	}
	...
	public setWrappingSettings(fontInfo: FontInfo, wrappingStrategy: 'simple' | 'advanced', wrappingColumn: number, wrappingIndent: WrappingIndent): boolean {
		...
		if (onlyWrappingColumnChanged) {
			previousLineBreaks = [];
			for (let i = 0, len = this.modelLineViews.length; i < len; i++) {
				previousLineBreaks[i] = this.modelLineViews[i].getLineBreakData();
			}
		}
	...
	public onModelLinesDeleted(versionId: number | null, fromLineNumber: number, toLineNumber: number): viewEvents.ViewLinesDeletedEvent | null {
		...
		let outputFromLineNumber = (fromLineNumber === 1 ? 1 : this.modelLineViewsLineCounts.getPrefixSum(fromLineNumber - 2) + 1);
		let outputToLineNumber = this.modelLineViewsLineCounts.getPrefixSum(toLineNumber - 1);

		this.modelLineViews.splice(fromLineNumber - 1, toLineNumber - fromLineNumber + 1);
		this.modelLineViewsLineCounts.removeValues(fromLineNumber - 1, toLineNumber - fromLineNumber + 1);
	...
	public onModelLinesInserted(versionId: number | null, fromLineNumber: number, _toLineNumber: number, lineBreaks: (LineBreakData | null)[]): viewEvents.ViewLinesInsertedEvent | null {
		...
		const isInHiddenArea = (fromLineNumber > 2 && !this.modelLineViews[fromLineNumber - 2].isVisible());

		let outputFromLineNumber = (fromLineNumber === 1 ? 1 : this.modelLineViewsLineCounts.getPrefixSum(fromLineNumber - 2) + 1);

		let totalOutputLineCount = 0;
		let insertLines: IModelLineView[] = [];
		...
		this.modelLineViews = this.modelLineViews.slice(0, fromLineNumber - 1).concat(insertLines).concat(this.modelLineViews.slice(fromLineNumber - 1));

		this.modelLineViewsLineCounts.insertValues(fromLineNumber - 1, insertPrefixSumValues);
	...
	public onModelLineChanged(versionId: number | null, lineNumber: number, lineBreakData: LineBreakData | null): [boolean, viewEvents.ViewLinesChangedEvent | null, viewEvents.ViewLinesInsertedEvent | null, viewEvents.ViewLinesDeletedEvent | null] {
		...
		let oldOutputLineCount = this.modelLineViews[lineIndex].getViewLineCount();
		let isVisible = this.modelLineViews[lineIndex].isVisible();
		let line = createSplitLine(lineBreakData, isVisible);
		this.modelLineViews[lineIndex] = line;
		let newOutputLineCount = this.modelLineViews[lineIndex].getViewLineCount();
		...
		if (oldOutputLineCount > newOutputLineCount) {
			changeFrom = (lineNumber === 1 ? 1 : this.modelLineViewsLineCounts.getPrefixSum(lineNumber - 2) + 1);
		...
		this.modelLineViewsLineCounts.setValue(lineIndex, newOutputLineCount);
	...
	public acceptVersionId(versionId: number): void {
		...
		if (this.modelLineViews.length === 1 && !this.modelLineViews[0].isVisible()) {
	...
	public getViewLineCount(): number {
		return this.modelLineViewsLineCounts.getTotalSum();
	}
	...
	private getViewLineInfo(viewLineNumber: number): ViewLineInfo {
		viewLineNumber = this._toValidViewLineNumber(viewLineNumber);
		let r = this.modelLineViewsLineCounts.getIndexOf(viewLineNumber - 1);
	...
	private getMinColumnOfViewLine(viewLineInfo: ViewLineInfo): number {
		return this.modelLineViews[viewLineInfo.modelLineNumber - 1].getViewLineMinColumn(
			this.model,
	...
	private getModelStartPositionOfViewLine(viewLineInfo: ViewLineInfo): Position {
		const line = this.modelLineViews[viewLineInfo.modelLineNumber - 1];
	...
	private getViewLineInfosGroupedByModelRanges(viewStartLineNumber: number, viewEndLineNumber: number): ViewLineInfoGroupedByModelRange[] {
		...
		for (let curModelLine = startViewLine.modelLineNumber; curModelLine <= endViewLine.modelLineNumber; curModelLine++) {
			const line = this.modelLineViews[curModelLine - 1];
	...
	public getViewLinesIndentGuides(viewStartLineNumber: number, viewEndLineNumber: number): number[] {
		...
		for (let modelLineIndex = modelStartLineIndex; modelLineIndex <= modelEndLineIndex; modelLineIndex++) {
			const line = this.modelLineViews[modelLineIndex];
			if (line.isVisible()) {
	...
	public getViewLineContent(viewLineNumber: number): string {
		...
		return this.modelLineViews[info.modelLineNumber - 1].getViewLineContent(this.model, info.modelLineNumber, info.modelLineWrappedLineIdx);
	}

	public getViewLineLength(viewLineNumber: number): number {
		const info = this.getViewLineInfo(viewLineNumber);
		return this.modelLineViews[info.modelLineNumber - 1].getViewLineLength(this.model, info.modelLineNumber, info.modelLineWrappedLineIdx);
	}

	public getViewLineMinColumn(viewLineNumber: number): number {
		const info = this.getViewLineInfo(viewLineNumber);
		return this.modelLineViews[info.modelLineNumber - 1].getViewLineMinColumn(this.model, info.modelLineNumber, info.modelLineWrappedLineIdx);
	}

	public getViewLineMaxColumn(viewLineNumber: number): number {
		const info = this.getViewLineInfo(viewLineNumber);
		return this.modelLineViews[info.modelLineNumber - 1].getViewLineMaxColumn(this.model, info.modelLineNumber, info.modelLineWrappedLineIdx);
	}

	public getViewLineData(viewLineNumber: number): ViewLineData {
		const info = this.getViewLineInfo(viewLineNumber);
		return this.modelLineViews[info.modelLineNumber - 1].getViewLineData(this.model, info.modelLineNumber, info.modelLineWrappedLineIdx);
	}
	...
	public getViewLinesData(viewStartLineNumber: number, viewEndLineNumber: number, needed: boolean[]): ViewLineData[] {
		...
		let start = this.modelLineViewsLineCounts.getIndexOf(viewStartLineNumber - 1);
		...
		for (let modelLineIndex = startModelLineIndex, len = this.model.getLineCount(); modelLineIndex < len; modelLineIndex++) {
			let line = this.modelLineViews[modelLineIndex];
			if (!line.isVisible()) {
				continue;
			}
	...
	public validateViewPosition(viewLineNumber: number, viewColumn: number, expectedModelPosition: Position): Position {
		...
		let line = this.modelLineViews[lineIndex];
	...
	public convertViewPositionToModelPosition(viewLineNumber: number, viewColumn: number): Position {
		...
		let inputColumn = this.modelLineViews[info.modelLineNumber - 1].getModelColumnOfViewPosition(info.modelLineWrappedLineIdx, viewColumn);
	...
	public convertModelPositionToViewPosition(_modelLineNumber: number, _modelColumn: number, affinity: PositionAffinity = PositionAffinity.None): Position {
		...
		while (lineIndex > 0 && !this.modelLineViews[lineIndex].isVisible()) {
			lineIndex--;
			lineIndexChanged = true;
		}
		if (lineIndex === 0 && !this.modelLineViews[lineIndex].isVisible()) {
			// Could not reach a real line
			// console.log('in -> out ' + inputLineNumber + ',' + inputColumn + ' ===> ' + 1 + ',' + 1);
			return new Position(1, 1);
		}
		const deltaLineNumber = 1 + (lineIndex === 0 ? 0 : this.modelLineViewsLineCounts.getPrefixSum(lineIndex - 1));

		let r: Position;
		if (lineIndexChanged) {
			r = this.modelLineViews[lineIndex].getViewPositionOfModelPosition(deltaLineNumber, this.model.getLineMaxColumn(lineIndex + 1), affinity);
		} else {
			r = this.modelLineViews[inputLineNumber - 1].getViewPositionOfModelPosition(deltaLineNumber, inputColumn, affinity);
		}
	...
	*/
	...
	public getViewLineNumberOfModelPosition(modelLineNumber: number, modelColumn: number): number {
		...
		if (this.modelLineViews[lineIndex].isVisible()) {
			// this model line is visible
			const deltaLineNumber = 1 + (lineIndex === 0 ? 0 : this.modelLineViewsLineCounts.getPrefixSum(lineIndex - 1));
			return this.modelLineViews[lineIndex].getViewLineNumberOfModelPosition(deltaLineNumber, modelColumn);
		}
		...
		const deltaLineNumber = 1 + (lineIndex === 0 ? 0 : this.modelLineViewsLineCounts.getPrefixSum(lineIndex - 1));
		return this.modelLineViews[lineIndex].getViewLineNumberOfModelPosition(deltaLineNumber, this.model.getLineMaxColumn(lineIndex + 1));
	}
	...
	public getInjectedTextAt(position: Position): InjectedText | null {
		...
		return this.modelLineViews[info.modelLineNumber - 1].getInjectedTextAt(info.modelLineWrappedLineIdx, position.column);
	}

	normalizePosition(position: Position, affinity: PositionAffinity): Position {
		const info = this.getViewLineInfo(position.lineNumber);
		return this.modelLineViews[info.modelLineNumber - 1].normalizePosition(this.model, info.modelLineNumber, info.modelLineWrappedLineIdx, position, affinity);
	}
...
class VisibleIdentityModelLineView implements IModelLineView {

	public static readonly INSTANCE = new VisibleIdentityModelLineView();
	...
	public setVisible(isVisible: boolean): IModelLineView {
		if (isVisible) {
			return this;
		}
		return InvisibleIdentityModelLineView.INSTANCE;
	}
...
class InvisibleIdentityModelLineView implements IModelLineView {

	public static readonly INSTANCE = new InvisibleIdentityModelLineView();
	...
	public setVisible(isVisible: boolean): IModelLineView {
		...
		return VisibleIdentityModelLineView.INSTANCE;
	}
...
export class ModelLineView implements IModelLineView {
...
function createSplitLine(lineBreakData: LineBreakData | null, isVisible: boolean): IModelLineView {
	if (lineBreakData === null) {
		...
		return new ModelLineView(lineBreakData, isVisible);
	}
}
